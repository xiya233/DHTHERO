...
import { SitePreferenceControls } from "@/components/site-preference-controls";
import { getFeatures, getSiteContent } from "@/lib/api";
import { getCopy } from "@/lib/i18n";
import { getServerSitePreferences } from "@/lib/site-preferences-server";
import { SITE_THEME_KEY } from "@/lib/site-preferences";
import "./globals.css";
...
const THEME_BOOTSTRAP_SCRIPT = `(() => {
  const key = "${SITE_THEME_KEY}";
  try {
    const cookiePair = document.cookie.split("; ").find((item) => item.startsWith(key + "="));
    const fromCookie = cookiePair ? decodeURIComponent(cookiePair.split("=").slice(1).join("=")) : null;
    const fromStorage = window.localStorage.getItem(key);
    const raw = fromCookie || fromStorage;
    const theme = raw === "dark" || raw === "light"
      ? raw
...
export default async function RootLayout({
  ...
  const [{ locale, theme }, features, siteContent] = await Promise.all([
        ...
        <div className="min-h-screen grid-paper">
          <header className="top-0 z-50 w-full max-w-none border-b-4 border-ink bg-paper px-6 py-4">
            <div className="mx-auto flex w-full max-w-6xl items-center justify-between">
              ...
              <SitePreferenceControls initialLocale={locale} initialTheme={theme} />
            </div>
          </header>
